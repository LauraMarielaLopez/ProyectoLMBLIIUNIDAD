...
namespace Datos
{
    public  class UsuarioDatos
    {
            ...
            try
            {
                ...
                using (MySqlConnection _conexion = new MySqlConnection(CadenaConexion.Cadena))
                {
                    ...
                    using (MySqlCommand comando = new MySqlCommand(sql, _conexion))
                    {
                        ...
                        comando.Parameters.Add("@Clave", MySqlDbType.VarChar, 120).Value = Codigo;
                    
                        valido = Convert.ToBoolean(await comando.ExecuteNonQueryAsync());   
                    }
                }
            }
            catch (Exception ex)
...

        {
            ...
            try
            {
                string sql = "INSERT INTO usuario VALUES (@Codig, @Nombre, @Clave, @Correo, @Rol, @EstaActivo);";
                using (MySqlConnection _conexion = new MySqlConnection(CadenaConexion.Cadena))
                {
                    await _conexion.OpenAsync();
                    using (MySqlCommand comando = new MySqlCommand(sql, _conexion))
                    {
                        ...
                        comando.Parameters.Add("@Nombre", MySqlDbType.VarChar, 20).Value = usuario.Nombre;
